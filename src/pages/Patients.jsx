...
export default function Patients() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    fetch(`http://localhost:3000/api/v1/patients?searchTerm=${search}`)
      .then((res) => res.json())
      .then((data) => setData(data.data));
  };
  const handleSelect = (value) => {
    if (value === "O+" || value === "B+" || value === "A+" || value === "AB+") {
      ...
      fetch(
        `http://localhost:3000/api/v1/patients?searchTerm=${value.slice(
          0,
          -1
        )}%2B`
      )
    ...
    <>
      ...
      <Table dataSource={data} style={{ marginTop: "30px" }}>
        ...
        <Column
          ...
          render={(tag) => (
            <>
              <Tag color={tag === "Healthy" ? "blue" : "red"} key={tag}>
                {tag}
              </Tag>
            </>
          )}
